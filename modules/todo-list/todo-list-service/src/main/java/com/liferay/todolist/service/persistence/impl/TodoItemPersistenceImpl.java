...
public class TodoItemPersistenceImpl
	 ...
	 */
	@Override
	public List<TodoItem> filterFindByGroupId(long groupId) {
	 ...
	 */
	@Override
	public List<TodoItem> filterFindByGroupId(
		long groupId, int start, int end) {
	...
	public List<TodoItem> filterFindByGroupId(
		...
		if (getDB().isSupportsInlineDistinct()) {
			sb.append(_FILTER_SQL_SELECT_TODOITEM_WHERE);
		}
		else {
			sb.append(_FILTER_SQL_SELECT_TODOITEM_NO_INLINE_DISTINCT_WHERE_1);
		}

		sb.append(_FINDER_COLUMN_GROUPID_GROUPID_2);

		if (!getDB().isSupportsInlineDistinct()) {
			sb.append(_FILTER_SQL_SELECT_TODOITEM_NO_INLINE_DISTINCT_WHERE_2);
		}
		...
		if (orderByComparator != null) {
			...
			if (getDB().isSupportsInlineDistinct()) {
				...
				sb.append(TodoItemModelImpl.ORDER_BY_SQL);
			}
		}

		String sql = InlineSQLHelperUtil.replacePermissionCheck(
			sb.toString(), TodoItem.class.getName(),
		...
		try {
			...
			if (getDB().isSupportsInlineDistinct()) {
				sqlQuery.addEntity(_FILTER_ENTITY_ALIAS, TodoItemImpl.class);
			}
			else {
				sqlQuery.addEntity(_FILTER_ENTITY_TABLE, TodoItemImpl.class);
			}
			...
			return (List<TodoItem>)QueryUtil.list(
	 ...
	 */
	@Override
	public TodoItem[] filterFindByGroupId_PrevAndNext(
			long todoItemId, long groupId,
			OrderByComparator<TodoItem> orderByComparator)
		throws NoSuchTodoItemException {
	...
	protected TodoItem filterGetByGroupId_PrevAndNext(
		Session session, TodoItem todoItem, long groupId,
		OrderByComparator<TodoItem> orderByComparator, boolean previous) {
		...
		List<TodoItem> list = sqlQuery.list();
	...
	public int filterCountByGroupId(long groupId) {
		...
		sb.append(_FILTER_SQL_COUNT_TODOITEM_WHERE);
	...
	private static final String _FILTER_SQL_SELECT_TODOITEM_WHERE =
		"SELECT DISTINCT {todoItem.*} FROM TodoList_TodoItem todoItem WHERE ";

	private static final String
		_FILTER_SQL_SELECT_TODOITEM_NO_INLINE_DISTINCT_WHERE_1 =
			"SELECT {TodoList_TodoItem.*} FROM (SELECT DISTINCT todoItem.todoItemId FROM TodoList_TodoItem todoItem WHERE ";

	private static final String
		_FILTER_SQL_SELECT_TODOITEM_NO_INLINE_DISTINCT_WHERE_2 =
			") TEMP_TABLE INNER JOIN TodoList_TodoItem ON TEMP_TABLE.todoItemId = TodoList_TodoItem.todoItemId";

	private static final String _FILTER_SQL_COUNT_TODOITEM_WHERE =
		"SELECT COUNT(DISTINCT todoItem.todoItemId) AS COUNT_VALUE FROM TodoList_TodoItem todoItem WHERE ";

	private static final String _FILTER_ENTITY_ALIAS = "todoItem";

	private static final String _FILTER_ENTITY_TABLE = "TodoList_TodoItem";

	private static final String _ORDER_BY_ENTITY_ALIAS = "todoItem.";

	private static final String _ORDER_BY_ENTITY_TABLE = "TodoList_TodoItem.";
